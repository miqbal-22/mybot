let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Sabar ngentod, Masih ada yang sedang membuat\nTeks Harta Tahta\ndi chat ini... tunggu sampai selesai anjink'
  else conn.hartatahta[m.chat] = true
  m.reply('_Sedang membuat..._\n*Mohon tunggu sekitar 1 menit, Sabar yaa ngentod*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '*© JINGAN*\nMade with FFmpeg', m)
  } finally {
