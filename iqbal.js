...
global.owner = ['6285745376798'] // Put your number here
global.mods = ['6285745376798'] // Want some help?
global.prems = ['6285745376798'] // Premium user has unlimited limit
...
setInterval(async () => {
  ...
  if (JSON.stringify(global.DATABASE.data) == lastJSON) conn.logger.info('Database is up to date')
    ...
    conn.logger.info('Done saving database, YouTube: Iqbal XD!')
  ...
  try {
    ...
    m.exp += 1
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (xp > 99) m.reply('Ngecit anjink -_-') // Hehehe
        else m.exp += xp
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < m.limit * 1 && plugin.limit) {
          this.reply(m.chat, `Limit lu habis, silahkan beli melalui *${usedPrefix}buy*`, m)
        ...
        try {
          ...
          if (m.limit) m.reply(+ m.limit + ' Limit terpakai yaa tod')
        }
  			break
  		}
  	}
...
conn.welcome = 'Halloo ngentoters, @user!\nSelamat datang di grup @subject'
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Lu bukan Owner jadi jangan so keras ngentod, Perintah ini hanya dapat digunakan oleh Owner!',
    owner: 'Lu bukan Owner jadi jangan so keras ngentod, Perintah ini hanya dapat digunakan oleh Owner!',
    mods: 'Lu bukan Moderator anjink, Perintah ini hanya dapat digunakan oleh Moderator!',
    premium: 'Ini khusus Elite dan member premium jadi jan so keras ngentod, Perintah ini hanya untuk member Premium!',
    group: 'Jangan TOLOL dehh lu lagi gk di grup asu, Perintah ini hanya dapat digunakan di Grup!',
    private: 'Gaseneng? PC ajjh ngentod, Perintah ini hanya dapat digunakan di Chat Pribadi!',
    admin: 'Lu itu cuma member jadi jangan sok keras, Perintah ini hanya untuk admin grup!',
